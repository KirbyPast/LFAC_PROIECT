...
%left AND
%%
progr : classes globals functions
          {                          
...
decl       :     TYPE ID ';' { 
               ...
               | ID ID ASSIGN '{' call_list '}' ';' { 
                              if(!current->existsId($2)) {
                                    current->addVar($1,$2,(std::string)"0");
                              } else {
                                   errorCount++; 
                                   yyerror("Variable already defined");
                              }
        ...
        if(current->getRoot() == "global"){
            ...
            if($3->evaluate() == "invalid") {
                ...
                if(current->getType($1) == $3->type ){
                    ...
                    errorCount++;
                    snprintf(error_message,256,"Attempting to assign value of type %s to variable of type %s!",$3->type.c_str(),current->getType($1).c_str());
